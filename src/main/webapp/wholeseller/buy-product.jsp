...
try (Connection conn = PostgresConnection.getConnection()) {
		...
		String sql = "SELECT shop_name  FROM wholeseller.wholeseller WHERE wholeseller_id = ? ";
...
try (Connection conn = PostgresConnection.getConnection()) {
		...
		String sql = "SELECT s.stock_id, s.product_name, s.price, sc.category_name, wh.shop_name  FROM wholeseller.stock s JOIN wholeseller.stock_cat sc ON s.stock_cat_id = sc.stock_cat_id JOIN wholeseller.wholeseller wh ON s.wholeseller_id = wh.wholeseller_id";
		PreparedStatement ps = conn.prepareStatement(sql);
		...
		ResultSet rs = ps.executeQuery();
		 ...
		 while (rs.next()) {
		        ...
		        String imageUrl = "https://via.placeholder.com/300x180?text=" + productName.replace(" ", "+");
...
%>
      ...
      <div class="manufacturer">Manufacturer: <%= manufacturer %></div>
     <a href="buy-pay-product.jsp?stockID=<%= stockID %>" ><button class="add-to-cart">Buy</button></a>
    </div>
<%
    } // end while
} catch (Exception e) {
