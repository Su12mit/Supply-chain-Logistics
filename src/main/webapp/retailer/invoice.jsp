...
try (Connection conn = PostgresConnection.getConnection()) {
		...
		String sql = "SELECT t.transport_name, c.customer_name, c.customer_address, dr.buyer_name,  dr.delivery_address, dr.buyer_email, dr.product_name, dr.quantity,  dr.price, dr.request_date, iv.invoice_id, iv.amount, riv.transaction_id, riv.tr_date_time, riv.mode_of_payment FROM transport.shipment s JOIN transport.transport t ON s.transport_id = t.transport_id JOIN transport.delivery_request dr ON s.delivery_request_id = dr.delivery_request_id  JOIN transport.customer c ON dr.customer_id = c.customer_id JOIN transport.invoices iv  ON s.shipment_id = iv.shipment_id JOIN retailer.invoice riv ON iv.invoice_id = riv.transport_invoiceid WHERE iv.invoice_id = ?";
...
<head>
    ...
    .container {
        ...
        padding: 30px;
        border-radius: 12px;
        box-shadow: 0 6px 12px rgba(0, 64, 0, 0.1);
    }
