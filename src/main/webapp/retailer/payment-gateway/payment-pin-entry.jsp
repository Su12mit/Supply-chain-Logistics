...
if (paymentSuccess == null) {
    ...
    paymentSuccess = "true".equals(successParam);
}
%>
...
try (Connection conn = PostgresConnection.getConnection()) {
		...
		String sql = "SELECT rtp.shop_name, rtp.bank_name, rtp.account_no, rtp.account_pin, iv.wholesaler_name, iv.amount, wh.bank_name AS wholesellerbank FROM retailer.retailer rtp JOIN retailer.invoice iv ON iv.retailer_id = rtp.retailer_id JOIN wholeseller.wholeseller wh ON iv.wholeseller_id = wh.wholeseller_id WHERE iv.order_id = ? ";
		...
		while (rs.next()) {
			...
			String Bankname = rs.getString("bank_name");
    		 ...
    		 int pin = rs.getInt("account_pin");
    		 String tname = rs.getString("wholesaler_name");
    		 String Bank = rs.getString("wholesellerbank");
...
<body>
  ...
  <div class="container" id="paymentBox" style="<%= (paymentSuccess ? "display:none;" : "display:block;") %>">
...
<form method="post" action="/Supply-chain-and-Logistic/RTPayStoreServlet" id="paymentForm">
  ...
  <div class="container success-box" id="successBox" style="<%= (paymentSuccess ? "display:block;" : "display:none;") %>">
    <div class="checkmark">Completed</div>
    <div class="success-text">Payment Successful</div>
   <div class="paid-amount" id="paidAmount">Rs. <%= request.getAttribute("amount") %></div>
<div id="dateTime" style="color: #666; font-size: 15px; margin-bottom: 20px;">
  <%= request.getAttribute("txnDateTime") %>
</div>
<div class="receipt" id="transactionDetails">
  <div><strong>Transaction ID:</strong><br> <%= request.getAttribute("txnId") %></div>
  <div><strong>FROM:</strong><br>
    <%= request.getAttribute("Shopname") %><br>
    <%= request.getAttribute("Bankname") %><br>
    Account No: <%= request.getAttribute("accountNo") %>
  </div>
  <div><strong>TO:</strong><br>
    <%= request.getAttribute("tname") %><br>
  ...
  window.onpopstate = function () {
    // User pressed Back button, redirect to index page
    window.location.href = "/Supply-chain-and-Logistic/retailer/index.jsp";
  };
</script>

  <audio id="successSound" src="pay_audio.mp3" preload="auto"></audio>
<script>
...
const serverPin = "<%= String.format("%04d", request.getAttribute("pin")) %>";

let attempts = 0; // Tracks attempts within the page session (refresh resets this)
...
function submitPIN() {
  ...
  if (pin !== serverPin) {
    attempts++;
    if (attempts >= 3) {
      alert("Try again after 48 hours.");
      window.location.href = "/Supply-chain-and-Logistic/All_login.jsp"; // Redirect after 3 failed attempts
      return;
    } else {
      alert("Incorrect PIN. Re-enter PIN. Attempt " + attempts + " of 3.");
  ...
  // If PIN is correct, proceed with processing payment
